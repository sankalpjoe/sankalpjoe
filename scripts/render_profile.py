...
def card(project, repo, index, mobile=False):
    ...
    state=status(repo)
    b+=text(w-28,37,state.upper() if state in ("Archived","Empty","Fork") else "↗",16,MUTED,mono=True,extra='text-anchor="end"')
    if mobile:
...
def project_groups(data, config):
    """Curated cards first; every other public project gets a truthful card."""
    repos={r["name"]:r for r in data["repos"] if r["name"]!=data["username"]}
    featured=[p for p in config["projects"] if p["repo"] in repos and repos[p["repo"]]["size"]]
    selected={p["repo"] for p in featured}
    remaining=[r for r in repos.values() if r["name"] not in selected]
    remaining.sort(key=lambda r:r["pushed_at"] or "",reverse=True)
    remaining.sort(key=lambda r:r["archived"])
    colors=(MINT,"#b9a4ff","#91c7ff","#e7ba83")
    extras=[]
    for repo in remaining:
        name=repo["name"]
        description=("No public files yet. This card will update when content is pushed."
                     if not repo["size"] else repo["description"] or "Explore the source and documentation on GitHub.")
        if len(description)>125:
            description=description[:124].rsplit(" ",1)[0]+"…"
        title=name.replace("-"," ").replace("_"," ")
        if len(title)>31:
            title=title[:30].rsplit(" ",1)[0]+"…"
        extras.append({"repo":name,"title":title,"subtitle":"Public repository / "+status(repo),
                       "description":description,"tags":[repo["language"] or "Explore on GitHub",status(repo)],
                       "category":"PUBLIC REPOSITORY","art":"signal",
...
def markdown(data,config):
    ...
    featured,extras=project_groups(data,config)
    ...
    parts=['<!-- Generated by scripts/refresh.py. Edit profile.json and scripts/render_profile.py. -->',
           ...
           '<p align="center">\n<a href="#selected-work">Selected work</a> &nbsp; / &nbsp; <a href="#more-public-repositories">More public repos</a> &nbsp; / &nbsp; <a href="#code-spectrum">Code spectrum</a> &nbsp; / &nbsp; <a href="#repository-index">Full index</a>\n</p>',
           ...
           f'<sub>Public repository data last synced: {stamp}. <a href="https://github.com/sankalpjoe?tab=repositories">See the live repository list</a>.</sub>',
    ...
    if extras:
        parts += ['## More public repositories', 'These panels follow the public repository list automatically. Empty and archived repositories are labelled.']
        for p in extras:
            parts.append(picture("project-"+p["repo"],p["repo"]+" — "+p["description"],repos[p["repo"]]["html_url"]))
...
def render(data):
    ...
    for mobile in (False,True):
        ...
        repos={r["name"]:r for r in data["repos"]}
        featured,extras=project_groups(data,config)
        for i,p in enumerate(featured+extras,1):
